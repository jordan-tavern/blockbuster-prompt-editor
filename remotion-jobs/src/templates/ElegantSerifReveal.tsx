...
interface ElegantSerifRevealProps {
  ...
  position?: Position;
  heavyFont?: boolean;
  /** Custom per-word reveal timing (frames from component start). Overrides uniform stagger. */
  wordDelays?: number[];
}
...
export const ElegantSerifReveal: React.FC<ElegantSerifRevealProps> = ({
  ...
  const renderWord = (
    ...
    const wordStart = wordDelays ? (wordDelays[index] ?? index * wordDelay + 8) : index * wordDelay + 8;
    ...
    return (
      <span
        ...
        style={{
          ...
          fontWeight: isLine2 ? 400 : heavyFont ? 800 : 500,
